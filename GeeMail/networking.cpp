#include "networking.h"
#include <cstring>
#include <vector>

#include <iostream>

CNetworkingServer::CNetworkingServer()
{
    isock = 0;
    iaddrinfo = 0;
    iaccepted_sock = 0;
    strport_no = "7788";

    std::memset(&hints, 0, sizeof(hints));
    hints.ai_family = AF_UNSPEC;
    hints.ai_socktype = SOCK_STREAM;
    hints.ai_flags = AI_PASSIVE;
    addr_size = sizeof(client_addr);
}

int CNetworkingServer::blisten()
{
    if ((iaddrinfo = getaddrinfo(NULL, strport_no.c_str(), &hints, &server)) != 0)
    {
        //std::cerr << "Error: get address info error: " << gai_strerror(iaddrinfo) << std::endl;
        return 1;
    }

    auto p = server;
    for (p; p != NULL; p = p->ai_next)
    {
        if ((isock = socket(p->ai_family, p->ai_socktype, p->ai_protocol)) < 0)
        {
            //std::perror ("Socket create failed: ");
            continue;
        }

        if (bind(isock, p->ai_addr, p->ai_addrlen) < 0)
        {
            //std::perror ("Binding socket failed: ");
            close(isock);
            continue;
        }

        break;
    }

    if (p == NULL)
    {
        //std::cerr << "Fail to bind socket." << std::endl;
        return 2;
    }

    listen(isock, 5);
    return 0;
}

int CNetworkingServer::baccept()
{
    if ((iaccepted_sock = accept(isock, &client_addr, &addr_size)) < 0)
    {
        iaccepted_sock = 0;
        return 1;
    }
    else
    {
        return 0;
    }
}

std::string CNetworkingServer::receive(const int MAX_BUFF_LENGTH)
{
    int irecv_buff_length = 0;
    std::vector <char> buff(MAX_BUFF_LENGTH);
    std::string RecvMsg;

    std::memset(buff.data(), 0, buff.size());
    while ((irecv_buff_length = recv(iaccepted_sock, buff.data(), buff.size(), 0)) > 0)
    {
        RecvMsg.append(buff.begin(), buff.begin() + irecv_buff_length);
        std::cout.write(buff.data(), irecv_buff_length);
        if (RecvMsg.find("\r\n\r\n") != std::string::npos)
            break;
    }

    return RecvMsg;
}

int CNetworkingServer::send(std::string SendMsg)
{
    if (iaccepted_sock == 0)
        return 1;

    ::send(iaccepted_sock, SendMsg.c_str(), SendMsg.size(), 0);
    return 0;
}

CNetworkingClient::CNetworkingClient()
{
    std::memset(&hints, 0, sizeof(hints));
    hints.ai_family = AF_UNSPEC;
    hints.ai_socktype = SOCK_STREAM;

    ServerIPAddress = "127.0.0.1";
    strport_no = "7724";
}

int CNetworkingClient::connect()
{
    int iaddrinfo;
    if ((iaddrinfo = getaddrinfo(ServerIPAddress.c_str(), strport_no.c_str(), &hints, &server_info)) != 0)
    {
        //std::cerr << "Error: get address info error: " << gai_strerror(iaddrinfo) << std::endl;
        return 1;
    }

    auto p = server_info;
    for (p; p != NULL; p = p->ai_next)
    {
        if ((isock = socket(p->ai_family, p->ai_socktype, p->ai_protocol)) < 0)
        {
            //std::perror ("Socket create failed: ");
            continue;
        }

        if (::connect(isock, p->ai_addr, p->ai_addrlen) < 0)
        {
            //std::perror ("Connecting socket failed: ");
            ::close(isock);
            continue;
        }

        break;
    }

    if (p == NULL)
    {
        //std::cerr << "Fail to bind socket." << std::endl;
        return 2;
    }

    return 0;
}

std::string CNetworkingClient::receive(const int MAX_BUFF_LENGTH)
{
    int irecv_buff_length = 0;
    std::vector <char> buff(MAX_BUFF_LENGTH);
    std::string RecvMsg;

    std::memset(buff.data(), 0, buff.size());
    while ((irecv_buff_length = recv(isock, buff.data(), buff.size(), 0)) > 0)
    {
        RecvMsg.append(buff.begin(), buff.begin() + irecv_buff_length);
        //std::cout.write(buff.data(), irecv_buff_length);
        if (RecvMsg.find("SERVEREND\a\a") != std::string::npos)
        {
            RecvMsg = RecvMsg.substr(0, (RecvMsg.size() - std::string("SERVEREND\a\a").size()));
            break;
        }
    }

    return RecvMsg;
}

int CNetworkingClient::send(std::string SendMsg)
{
    if (isock == 0)
        return 1;

    ::send(isock, SendMsg.c_str(), SendMsg.size(), 0);
    return 0;
}
